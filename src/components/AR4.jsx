...
function AR4() {
  let { idar } = useParams();
  let { checkState4, checkStateRan4 } = useCheck();
  ...
  const getData = async () => {
    try {
      await axios.get(`${url}/checkpoint`).then((res) => {
        setLatutude1(res.data[0].latitude4);
        setLongitude1(res.data[0].longitude4);
        setLength(res.data[0].length4);
        const data = res.data[0];
        console.log(data);
        const urls = `${url}/checkpoint/s`;

        setMindFile(`${urls}/${data.cpId}/${data.startFile4}`);
        setGltfFile(`${urls}/${data.cpId}/${data.resultFile4}`);

        console.log(
          "setGltfFile : ",
          `${urls}/${data.cpId}/${data.startFile4}`
        );
        console.log("length", res.data[0].length4);
      });
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <>
      ...
      {isSuccess ? (
        <Link to="/ar_success/4">
          <div className="btn">
            <button className="btn-ar" onClick={(localStorage.getItem("isRandom") === "y") ? checkStateRan4: checkState4}>
              Next
            </button>
          </div>
        </Link>
