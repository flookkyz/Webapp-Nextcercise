...
function AR3() {
  let { idar } = useParams();
  let { checkState3, checkStateRan3 } = useCheck();
  ...
  const getData = async () => {
    try {
      await axios.get(`${url}/checkpoint`).then((res) => {
        setLatutude1(res.data[0].latitude3);
        setLongitude1(res.data[0].longitude3);
        setLength(res.data[0].length3);
        const data = res.data[0];
        console.log(data);
        const urls = `${url}/checkpoint/s`;

        setMindFile(`${urls}/${data.cpId}/${data.startFile3}`);
        setGltfFile(`${urls}/${data.cpId}/${data.resultFile3}`);

        console.log(
          "setGltfFile : ",
          `${urls}/${data.cpId}/${data.startFile3}`
        );
        console.log("length", res.data[0].length3);
      });
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <>
      ...
      {isSuccess ? (
        <Link to="/ar_success/3">
          <div className="btn">
            <button className="btn-ar" onClick={(localStorage.getItem("isRandom") === "y") ? checkStateRan3: checkState3}>
              Next
            </button>
          </div>
        </Link>
